...
test('round trip: Music app -> LyricLab -> Music app -> LyricLab loses nothing on either side and overclaims nothing', function () {
  ...
  var out1 = MP.exportJson(s, { frameworks: C.FRAMEWORKS, progression: C.load(s), selfReport: 'beginner', version: 'music-v0', device: 'phone', now: T0 });
  ...
  var uke = seen.doc.evidence.filter(function (e) { return e.id === 'ev:app:music:phone:progression:ukulele'; })[0];
  ...
  // And the app's own counters did not move from seeing its own progression evidence again (the import never feeds them):
  ...
  var out2 = MP.exportJson(s, { frameworks: C.FRAMEWORKS, progression: C.load(s), selfReport: 'beginner', version: 'music-v0', device: 'phone', now: T3 });
  ...
  var uke2 = d2.evidence.filter(function (e) { return e.id === 'ev:app:music:phone:progression:ukulele'; })[0];
...
test('a participant that strips what it does not understand is DETECTABLE: the merge keeps the local copy, and the trail shows who touched the doc', function () {
  ...
  var out = MP.exportJson(s, { frameworks: C.FRAMEWORKS, progression: C.load(s), selfReport: 'intermediate', device: 'phone', now: T0 });
...
test('a hand-back that EDITS the numbers inside the app\'s own progression evidence changes nothing the app believes: the record is preserved as data, but the app never re-ingests its counters from a profile', function () {
  var s = FakeStore();
  C.recordEvidence('ukulele', 'uke-open-chords', null, s);
  var out = JSON.parse(MP.exportJson(s, { frameworks: C.FRAMEWORKS, progression: C.load(s), device: 'phone', now: T0 }));
  ...
  // (songbook.js applyProfileDoc never touches competency.js).
  ...
  var again = JSON.parse(MP.exportJson(s, { frameworks: C.FRAMEWORKS, progression: C.load(s), device: 'phone', now: T3 }));
  assert.strictEqual(again.evidence.filter(function (e) { return e.id === 'ev:app:music:phone:progression:ukulele'; })[0].data.competencies[0].evidence_count, 1);
});
