...
> **RECONCILED 2026-07-10, second pass (clear-the-chessboard mission):** live version is **music-v132** (PR #198 squash-merged after the v121 first-pass reconcile below was drafted). The mission cleared the board: #197 (visual language) + #198 (progression-aware Studio picker + Mixolydian/Dorian + PW scenario suite + key-aware speller core) MERGED; #98 CLOSED as superseded (its `spellScaleKeyAware` core landed fresh + 14-tested in #198; render wiring is the in-flight FORK-4 PR); #70 CLOSED as superseded by M-0. #88 (M-3 strum, draft) is the one legacy PR left open. Usage flows are now committed declarative PW scenarios ([test/pw/](../../test/pw/README.md), 8 green). Mission goal spec: [goal-clear-chessboard-20260710](goal-clear-chessboard-20260710.md).

## NOW (in flight)

| Mission | State | Spec |
|---|---|---|
| **FORK-4 removal - key-aware render wiring** (the S-KEY-SPELLING finish): route chord names / scale list / fretboard through `spellScaleKeyAware`/`spellRootInKey`, flat key NAMES in the picker (Bb not A#), update canonical-sharp tests | IN FLIGHT this mission (core merged in #198; wiring PR follows) | [goal spec](goal-clear-chessboard-20260710.md) + S-KEY-SPELLING row below |
...
|---|---|---|---|
| S1 | Wave 2: S-TONES, S-ROMAN+S-KEYPOLICY, S-GOLDEN-B | UNBLOCKED when the FORK-4 wiring PR merges (the #98-merge trigger transfers to it - see NOW) | Fully specced in [ux-sprint-1-20260703.md](ux-sprint-1-20260703.md) items 7-9 + amendments - APPROVED |
| S2 | Sprint 2: M3 songs/tracks full merge | **5A RESOLVED (operator, 2026-07-04: dissolve the finder - curation to +Add/per-item edit, circle panel into the Studio)**; remaining gate: FORK-4 wiring PR merged + 30-min spec-formalize interview | Build plan in [UX-FRICTION-LOG](../../music/docs/UX-FRICTION-LOG.md) M3 section; council + chair sequencing decided; needs a 30-min goal-interview to spec-formalize -> then launch-ready |
...
| S-WHYNOTE-SCALE | **Studio "Why this scale works" banner keys to the KEY, not the selected scale** - after the Mixolydian default lands, the banner still reads "C major and its relative minor share the same notes." Correct but stale-feeling next to a Mixolydian selection; re-derive (or hide) the banner on scale-chip change, same reactive seam as renderJamPanel. Found in the #198 merge review (non-blocking). | in-Claude review of PR #198, 2026-07-10 |
