...
Engine amendments (same PR): FactStat gains `ok` (first-try-correct streak: `wrongs > 0 ? 0 : ok + 1`, except that `recordAnswer(stats, result, now, runStart)` leaves `ok` unchanged on a right answer when the fact's `last >= runStart`, so a streak moves at most once per workout); `normalizeCfg` keeps `skill` (`/^[a-z0-9-]{1,32}$/`) and `label` (string, <= 40 chars); `pool` scopes x and / to the picked tables even when coach is on (Custom's Coach clears the picks to mean "all"); `cfgKey` leads with `sk:<id>` for a skill set and keeps the tables for a coach set that has them (a coach set with no tables keeps v1's `coach` slot, so Custom coach bests survive); `cfgLabel` returns `label` when present.
