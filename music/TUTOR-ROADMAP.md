...
| 5 | **Song-form coaching** - AABA, intro/verse/chorus/bridge; guide building a coherent whole song from sections | PROTOTYPED (tutor sandbox, wave 4) - Verse/Chorus/Bridge with transition-quality-guided key picks. Not yet in the real Compose/Tracks app. |
...
**Wave 4 (`music/TUTOR-SONG-BUILDER-CHECKLIST.md`):** Phase 5 (song-form coaching), made real - the "what would Phase 5 need" discovery notes above are now `music/tutor/song-builder.js`. A song is 3 fixed sections (Verse, Chorus, Bridge); each section IS a `mini-compose.js` state (reducers work on it directly), and after each section the AI presents 3 candidate keys for the NEXT one - each pre-labeled with why it's a pleasing transition (dominant/subdominant/relative/shared pivot chord) via `analyzeTransition`, so picking a key is itself the theory lesson. Ends with a full song readout: every section's progression + roman numerals + the reasoning behind each transition. All new theory reuses `Circle.js` with zero new theory functions, exactly as scoped. Still a fixed 3-section shape (no arbitrary structure/reordering/timing) and still scripted.
