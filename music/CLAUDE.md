...
- **Note spelling is key-aware.** The scale note list AND the fretboard must agree (F major shows `Bb`, not `A#`); a label under `text-transform:uppercase` must not uppercase a flat (`Bb`→`BB`). See `Diagram.scale(opts.names)` + `.bt-st-notes`.
