...
- **Note spelling is key-aware, from ONE engine (#85, reverses FORK-4).** `circle.js` is the SSOT: flat keys render flats (F major shows `Bb`, not `A#`), sharp keys render sharps. The Compose chips + key picker (`songbook.js`) AND the Studio scale/chords/fretboard (`tracks.js`) DELEGATE to `Circle.diatonicKeyAware` / `scaleKeyAware` / `keyLabel` - never a parallel speller (`songbook.js` keeps only a circle-absent canonical fallback, like it already does for `MODE_STEPS`). The stored key VALUE stays canonical-sharp (transpose / filtering unchanged); only labels flip. A label under `text-transform:uppercase` must not uppercase an accidental letter (`.bt-st-notes` opts out). NOT yet key-aware (kept canonical): the circle-of-fifths WHEEL + the Library/Repertoire key facet. See `music/docs/THEORY-ENGINE.md` for the full contract + `Diagram.scale(opts.names)`.
...
- **SW cache version is part of the change, not a follow-up.** Whenever you touch ANY file listed in `music/sw.js` `CORE` (any profile, `songbook.js`, `tracks.js`, `nav-history.js`, `diagram.js`, `tuner.js`, `audio.js`, `songs.json`, the CSS, etc.), bump `CACHE = 'music-vN'` in the SAME commit. Skipping the bump means returning users keep the cached old asset until they manually clear it. Cheap to do; invisible-until-it-isn't to skip.
