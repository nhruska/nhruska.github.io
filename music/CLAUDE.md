...
- **Note spelling is KEY-AWARE (regime B, 2026-07-10 - FORK-4 retired).** Inside a stated key, note/chord NAMES spell by function (letter-per-degree from `Circle.preferredTonicName`): the bVII of C reads `Bb`, never `A#`, and the name always agrees with the degree label. **Chord TOKENS stay canonical-sharp** (storage, voicing packs, audio, suggestions) - only DISPLAY respells, via `dispChord`/`dispChordName`/`packDiagram(token, size, displayName)`. Keyless contexts (tuner, All-browse) stay sharp. Flat INPUT still normalizes for identity. A label under `text-transform:uppercase` must not uppercase an accidental letter (`.bt-st-notes` opts out). Full contract + the enforced professor traps: [engineering-wiki/theory-engine/note-spelling.md](engineering-wiki/theory-engine/note-spelling.md).
...
- **SW cache version is part of the change, not a follow-up.** Whenever you touch ANY file listed in `music/sw.js` `CORE` (any profile, `songbook.js`, `tracks.js`, `nav-history.js`, `diagram.js`, `tuner.js`, `audio.js`, `songs.json`, the CSS, etc.), bump `CACHE = 'music-vN'` in the SAME commit. Skipping the bump means returning users keep the cached old asset until they manually clear it. Cheap to do; invisible-until-it-isn't to skip. **The bump is a PAIR since M-SETTINGS-CLARITY:** also refresh `shared/build-stamp.js` (`VERSION` = the new CACHE string, `UPDATED_ISO` = now, UTC) in the same commit - `scripts/check-cache-bump.sh` fails on a stamp-less bump or a VERSION/CACHE drift, and `test/build-stamp.test.js` fails the suite on drift too. **And the version is PER ASSET-CHANGING COMMIT, not per PR (S-SW-PER-COMMIT, 2026-07-24):** the first asset-changing commit on a PR uses `music-v<PR#>`, and every LATER one appends a counter - `music-v306-2`, `-3`. Holding one version across a PR's follow-up commits is what makes a fix read as 'not working' on the phone: the device installed the preview at the first build, the cache key never changed, so the service worker kept serving that first build. PR #306 cost two UAT rounds to exactly this (the operator heard the original audio cutoff while the fixes sat undelivered). `check-cache-bump.sh` now FAILS when the newest asset-changing commit reuses a version an earlier commit already shipped, and WARNS on a historical reuse a later bump already superseded. **The build stamp in Settings is the verify handle** - read it on the device before judging a fix; if it does not match the version the PR comment names, you are looking at a cached older build. **Version = the PR number (operator directive 2026-07-20):** set `CACHE`/`VERSION` to `music-v<PR-number>` so the deployed build maps 1:1 to its PR (`v288` on the live site == PR #288 - a clean audit trail). Set it right after the PR is created (the number is known then); a same-branch follow-up push keeps that number. This also removes the old rebase re-bump dance: two open PRs never collide (PR numbers are unique) and the later PR's number is already higher, so whichever merges second needs no cache re-number - just rebase and resolve the pair to its own PR number.
- **ALL CSS is external (operator directive 2026-07-19).** App markup carries NO inline `<style>` blocks and no `style=` attributes - every rule lives in the shared stylesheets (`songbook.css` / `tracks.css` / `repertoire-form.css`). The one exception: `style="display:none"` INITIAL-state attributes that JS toggles via `el.style.display` (converting those breaks the toggle contract). Throwaway prototype artifacts under `docs/artifacts/` are exempt (not app source).
...
- **Test the real box.** Prefer live Playwright against the served app (the box OOMs on full e2e suites — run ONE scenario at a time, commit first). Node unit tests via `node test/run-all.js` must stay green. **Usage flows are committed declarative scenarios** in [test/pw/](../test/pw/README.md) (pw-replay JSON + runner) — run one on demand (`python3 test/pw/run-scenario.py test/pw/scenarios/<name>.json`) instead of hand-scripting Playwright; add new flows as scenario JSON, new step verbs to the runner.
- **Self-verify with one command: `python3 scripts/verify.py run`** (`--changed` default, `--feature <id>`, `--all`) - boots the site on a worktree port, runs the unit suite + cache-bump gate + each touched feature's checks/scenarios/probes from [.verify/manifest.json](../.verify/manifest.json), prints one JSON verdict (exit 0 pass / 1 fail / 2 config). `scripts/verify.py` is vendored - never edit it.
