...
| Theme (light/dark) tokens + boot + accent palette | [songbook.css](../../shared/songbook.css) :root + :root[data-theme="light"] blocks, resolved via [theme.js](../../shared/theme.js) Theme.effectiveTheme against the `music.theme.v1` localStorage key; Theme.PALETTE is the 8-swatch accent list | play/index.html pre-paint boot script, play/triad-inversions.html, Settings theme control, the `math/` sibling app (player accent colours) | Any new surface (including `docs/artifacts/*` packs outside `music/`) copies these token values and the boot-script pattern - never invent a different palette. A sibling app under the site root (e.g. `math/`) links `theme.js` directly instead of copying ([theme-tokens.md](theme-tokens.md)) |
