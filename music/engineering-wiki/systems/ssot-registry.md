...
| Note spelling (key-aware display; canonical tokens) | [circle.js](../../shared/circle.js) preferredTonicName/scaleInKey/diatonicInKey/soloScaleInKey/noteInKey plus ROOTS/spell() | display seams in tracks/songbook/key-explorer; token consumers in voicing/audio/suggestions | Inside a stated key, display names follow function; internal chord tokens and keyless contexts remain canonical-sharp |
...
| Theme (light/dark) tokens + boot | [songbook.css](../../shared/songbook.css) :root + :root[data-theme="light"] blocks, resolved via [theme.js](../../shared/theme.js) Theme.effectiveTheme against the `music.theme.v1` localStorage key | play/index.html pre-paint boot script, play/triad-inversions.html, Settings theme control | Any new surface (including `docs/artifacts/*` packs outside `music/`) copies these token values and the boot-script pattern - never invent a different palette ([theme-tokens.md](theme-tokens.md)) |
...
| Shape metadata | [shape-classify.js](../../shared/shape-classify.js) (PR #134; consumed by diagram-pref PR #143) | diagram-pref labels - BIG (maximize) renders ONLY since U25 (M-SETTINGS-CLARITY): small picker/strip cards never label | Never infer shape from a chord name; classify the actual frets; unclassifiable = honest null; the size gate lives at the chord-pack-adapter choke point, never per call site |
| Exclusive disclosure (accordion) | [accordion.js](../../shared/accordion.js) + `.accSec`/`.accBtn`/`.accBody` ([songbook.css](../../shared/songbook.css)) | play/index.html Settings sections | One-at-a-time disclosure group for PANEL surfaces; never hand-roll a second accordion (D6 still bars accordions from screen scrollers); guarded by [test/accordion.test.js](../../../test/accordion.test.js) |
| Build freshness stamp | [build-stamp.js](../../shared/build-stamp.js) VERSION + UPDATED_ISO | play/index.html Settings meta line, play/triad-inversions.html footer | VERSION is a deliberate guard-locked mirror of sw.js CACHE (pages cannot read a SW constant synchronously) - [check-cache-bump.sh](../../../scripts/check-cache-bump.sh) + [test/build-stamp.test.js](../../../test/build-stamp.test.js) fail on drift; authored at change time WITH every CACHE bump; BuildStamp.fmt() is the one datetime formatter (Settings last-run rows reuse it) |
| Suggestion table | [sugg.js](../../shared/sugg.js) Sugg.SUGG (PR #137; name-keyed, e.g. SUGG['C'] -> ['G','Am','F','Em','Dm']) | Compose next-chord chips, completions | Never build follower rules in a consumer; tokens remain canonical while display respells at the named seam |
...
| Pentatonic/blues key-aware spelling | circle.js `soloScaleInKey` | SHIPPED 2026-07-10; guarded by key-spelling tests |
