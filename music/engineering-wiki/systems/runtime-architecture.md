...
Load order is dependency order (music/play/index.html script block, ~lines 266-281): theme.js (pre-paint) -> nav-history -> queue -> tempo -> circle -> key-explorer -> list-item -> notables -> repertoire -> repertoire-form -> songbook -> tuner -> diagram -> audio -> tracks -> candidates -> inline bootstrap.
