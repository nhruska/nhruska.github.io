...
| `music.lastRestore.v1` | play/index.html (restore onConfirm, after `Backup.restore` succeeds) | ISO date string | No - device-local stamp | **No** - excluded (backup.js EXCLUDE) | **M-SETTINGS-CLARITY (2026-07-05).** The Settings Restore row's "Last restore" meta. Mirrors `music.lastBackup.v1`'s exclusion reasoning exactly: a restored envelope must not overwrite the DESTINATION device's own restore time. Additive key, defensive read, no migration needed |
| `music.notables.v1` | notables.js:57 | `{ consumerId: dismissedEpochMs }` | No - dismissal state | Yes | One-shot dismissible tip bookkeeping |
| `music.guidanceLevel.v1` | guidance-level.js (`KEY`, `get`/`set`) | `'beginner' \| 'intermediate' \| 'advanced'` | Yes - the musician's self-reported level | Yes (`music.` prefix) | **S-GUIDANCE (registered here by M-MUSICIAN-PROFILE-VNEXT, 2026-09-13, which found the row missing).** null = ask pending, never a silent default; only an explicit tap (welcome tour, Settings segment) writes it. Grades one-shot guidance tips; exported into profile.json as the app's ONE self-report assessment |
| `music.guidanceLevel.at.v1` | guidance-level.js (`AT_KEY`, `at`; written by `set`) | ISO 8601 | Yes - when the level was tapped | Yes (`music.` prefix) | **M-MUSICIAN-PROFILE-VNEXT (2026-09-13).** The self-report assessment's `at` is when the CLAIM was made, not when it was exported - otherwise a coach's later, better-grounded claim on the same branch could lose to a tap re-stamped at export. Absent on older devices -> the exporter falls back to its clock. Additive, no migration |
...
| `music.diagram.pref.v1` | diagram-pref.js (`KEY`, `get`/`set`) | `'dots'\|'patterns'` | No - pref | Yes (`music.` prefix) | **SHIPPED 2026-07-06** (S-DIAGRAM-PREF steps 1-2, see D-DIAGRAM-PREF, decisions.md). Additive key, defensive read, default 'dots' - shipped under the existing additive-vs-breaking law above with no `StorageMigrate` registration needed. Set via the `'diagrampref'` Notables prompt or the Settings "Fretboard diagrams" row (play/index.html) |
| `music.fretview.v1` | tracks.js (`readFretView`/`writeFretView`, `openStudio` region) | `'window'\|'full'` | No - pref | Yes (`music.` prefix) | **M-EAR wave 1.5 (U13, decisions.md D-EAR-1.5).** Additive key, defensive read/write, default 'window' - the Practice Studio's fretboard-view toggle (windowed pager vs the full 0-14 span). No `StorageMigrate` registration needed, same additive-vs-breaking law as `music.diagram.pref.v1` above |
| `music.tempo.v1` | tracks.js (`readTempo`/`writeTempo`, `openStudio` region) | `'slow'\|'med'\|'fast'` | No - pref | Yes (`music.` prefix) | **M-EAR wave 1.6 (U14, decisions.md D-EAR-1.6).** Additive key, defensive read/write, default `'med'` (104bpm) - the Practice Studio's Slow/Med/Fast scale-audition tempo control (`TEMPO_BPM = {slow:72, med:104, fast:140}`). Default changed from the implicit pre-U14 `'slow'` (72bpm) to `'med'` - the operator's own complaint was that the ONLY available speed was too slow, so the fix ships a faster default alongside the new control. No `StorageMigrate` registration needed, same additive-vs-breaking law as `music.fretview.v1` above |
| `music.competency.v1` | competency.js (`STORAGE_KEY`, `load`/`save`/`recordEvidence`) | `{ [frameworkId]: skill-competency-profile/v1 doc }` | **Yes** - the app's progression tracker | Yes (`music.` prefix) | **M-COMPETENCY (2026-07-11, #243)** - registered here by M-MUSICIAN-PROFILE (2026-09-13), which found the row missing. Levels 0-100 grown from compose evidence toward a target; the app's OWN ladder, not a proficiency claim. Additive key, defensive read, no `StorageMigrate` registration |
| `music.profile.v1` | musician-profile.js (`STORAGE_KEY`, `load`/`save`/`exportJson`/`importJson`) | one `musician-profile/v1` document | **Yes** - the person's lifelong profile | Yes (`music.` prefix) | **M-MUSICIAN-PROFILE (2026-09-13).** The person-owned document: taxonomy, assessments, evidence, goals, plan, participants, provenance, extensions + unknown keys preserved. Written on every bundle export (composed from the app's counters as compose-modality EVIDENCE + the self-report dated by its tap), merged on `profile.json` import. **VNEXT (2026-09-13):** the taxonomy floor adds `musicianship/*` + four transferable `stringed-instrument/*` ids (never observed by the app); assessments may carry `confidence`; `plan` gains `focus` + item `kind`/`deep_link`; the export retires the app's own legacy device-less progression records and folds duplicate routine provenance stamps. All additive - old documents normalize; no migration. Spec: [musician-profile.md](musician-profile.md) |
| `music.device.v1` | musician-profile.js (`DEVICE_KEY`, `deviceId`) | string (`dv_<8 hex>`) | No - device-local stamp | **No** - excluded (backup.js EXCLUDE) | **M-MUSICIAN-PROFILE (2026-09-13, volley 2).** THIS device's id, scoping the app's progression evidence inside profile.json (`ev:app:music:<device>:progression:<fw>`) so two devices' records coexist. Excluded for the same reason as `music.lastBackup.v1`: a restored envelope must not make the destination device impersonate the source |
