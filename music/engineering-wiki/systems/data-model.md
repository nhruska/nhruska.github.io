...
| `music.profile.v1` | musician-profile.js (`STORAGE_KEY`, `load`/`save`/`exportJson`/`importJson`) | one `musician-profile/v1` document | **Yes** - the person's lifelong profile | Yes (`music.` prefix) | **M-MUSICIAN-PROFILE (2026-09-13).** The person-owned document: taxonomy, assessments, evidence, goals, plan, participants, provenance, extensions + unknown keys preserved. Written on every bundle export (composed from the app's counters as compose-modality EVIDENCE + the self-report), merged on `profile.json` import. Additive key, defensive read (corrupt -> fresh blank), no migration. Spec: [musician-profile.md](musician-profile.md) |
