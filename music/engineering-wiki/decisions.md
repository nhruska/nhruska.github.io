...
| D-TARGET | Chord-tone targeting is intersection-only | A tapped chord's tones mark the fretboard ONLY where they also fall inside the current scale (`Circle.chordTones()` intersected against scale pcs) - a chord tone outside the scale (e.g. the major 3rd over a C7 target in C blues) is a deliberate deferral, never a ghost dot; the mentor card carries that advice in prose instead | [systems/practice-studio.md](systems/practice-studio.md) |
