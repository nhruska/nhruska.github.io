...
| Drag-to-reorder | one grammar across ALL reorder surfaces: `wireSlotDrag` (progression strip), `wireSectionDrag` (canvas section cards), `wireSetlistDrag` (setlist rows, 2026-07-19) | 300ms long-press lift on touch / 6px slop on mouse; `.dragging` opacity + insertion-edge marker (`.dropBefore`/`.dropAfter` - horizontal box-shadow on the chip strips, vertical on the setlist); trailing-click swallow. The up/dn arrows STAY as the a11y/keyboard-and-SR path - drag is always ADDITIVE, never a replacement. |
