<!-- GENERATED from music/engineering-wiki/: systems/data-model.md, systems/instrument-profiles.md | regenerated by re-synthesizing those pages | 2026-09-06 -->
...
Track records: `{ title, artist, genre, key (preferred tonic name), mode, bpm?, capo?, yt?|null, tags? }`. When `yt` is null, a deterministic YouTube search query substitutes.

`trackKey` is the stable identity for the curated-URL overlay. Its root normalization
folds enharmonic spellings, so Eb and D# identify the same track while stored JSON can
use the preferred tonic name. A `LEGACY_TRACKKEYS` migration map remaps keys stored
under old mode-coarsening rules.
...
**Enharmonic shape lookup:** canonical chord tokens enter the voicing seam, but a
profile may key voicings under flats. Lookup order: exact name -> enharmonic twin ->
movable-template fallback. A separate key-aware display name is preserved; only the
fingering source changes.
