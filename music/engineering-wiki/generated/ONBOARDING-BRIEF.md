<!-- GENERATED from music/engineering-wiki/: AGENTS.md, index.md (dev-audience brief over the wiki's read order) | regenerated by re-synthesizing those pages | 2026-09-06 -->
...
1. **Key-aware display spelling.** Inside a stated key, note and chord names spell by
   function; internal chord tokens and keyless contexts remain canonical-sharp. Track
   keys store preferred tonic names, while enharmonic identity stays folded. FORK-4 is
   retired. Full contract: [THEORY.md](THEORY.md).
...
| Question | Read |
|---|---|
| Why does F major display Bb while an internal token may be A#? | [THEORY.md](THEORY.md) |
...
key labels, token identity, or the key-aware display seams.
