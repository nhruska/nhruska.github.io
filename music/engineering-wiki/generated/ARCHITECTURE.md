<!-- GENERATED from music/engineering-wiki/: systems/runtime-architecture.md, systems/offline-pwa.md, systems/data-model.md, theory-engine/architecture.md | regenerated by re-synthesizing those pages | 2026-09-06 -->
...
Notable behaviors: **enharmonic tolerance** (a canonical chord token finds a flat-keyed
fingering via respelling retry without changing the key-aware display name),
**movable-voicing fallback** (closed shapes slide up the neck for transposed keys;
exact -> movable -> name-only), and **artifact-aware ukulele shapes** (C/F/A-shape
barre templates inject when the 4-string top-interval pattern matches, aligning I-IV-V
chains with the triad-inversions pedagogy).
...
Because the naming policy is isolated at named display seams, stated-key labels can be
key-aware while theory math and canonical chord tokens remain stable. Track-key storage
uses preferred tonic names and spelling-proof identity. Full contract: [THEORY.md](THEORY.md).
